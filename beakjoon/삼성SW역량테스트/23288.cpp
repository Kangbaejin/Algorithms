...
int moving(dice &dice){
    cout << "moving start on (x, y)" << dice.x << " " <<dice.y <<'\n';
    ...
    if(newr<0 || newr>=N || newc<0 || newc>= M){
        ...
        //A > B
        if(map[newr][newc] > dice.dice_plane[6]){
        ...
        //A < B
        else if(map[newr][newc] < dice.dice_plane[6]){
