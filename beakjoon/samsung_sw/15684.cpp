 ...
 * 
 * 실행시간이 0 ms 인 코드이다.
 * https://www.acmicpc.net/source/34302435
 * 
 * 여기선 32~35 번의 while 문 을 이용했고, main 내부에서 dfs 를 각각 val 값에 해당하게 실행하고 있다.
 * 이때 flag 를 통해 dfs 내에서 답을 1일때 먼저 확인 후, 1칸만 추가하여 답을 낼 수 있다면 1을 ans 에 저장하고,
 * 그렇지 않았다면 다음 dfs(0,2) 를 통해서 같은 논리를 전개한다.
 * 
 * 
 * 초기 구현 계획 당시에 내가 구상했던 방식으로 작동하는데, 이를 bfs 로 구현해야하는지 생각했었는데 이런식으로도
 * 구현하여 답을 얻을 수 있다는 것을 배울 수 있었다.
 * 
 */
